...
def tsp(graph, start_idx):
    ...
    for perm in itertools.permutations(vertex):
        ...
        if current_cost < min_cost:
            ...
            min_path = [start_idx] + list(perm) + [start_idx]
...
st.markdown(
    """
    Anda dapat memilih untuk memulai perjalanan dari gedung berikut:
    - TULT
    - TUCH
    - OpenLibrary
    - Gd. Cacuk
...
start_node = st.text_input("Ingin mulai dari mana?", "TULT")
if start_node not in nodes:
    st.write("Masukkan tidak valid! Silakan masukkan salah satu dari: 'TULT', 'TUCH', 'OpenLibrary', atau 'Gd. Cacuk'.")
else:
    ...
    if st.button("Mulai"):
        ...
        st.write(f"Waktu eksekusi: {timeRun:.6f} detik")

        # Membuat graf untuk menampilkan node
        route_G = nx.Graph()
